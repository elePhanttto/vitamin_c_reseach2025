...
def plot_posterior(samples,x_truth,epoch,idx,run='testing',all_other_samples=None):
    ...
    if all_other_samples is not None:
        ...
        # ========================================================
        # 【追加】表示するパラメータを絞り込むフィルター
        # ========================================================
        # ここに表示したいパラメータの名前（params.jsonのキー）を書く
        # 例: 質量1, 質量2, 距離, RA, DEC だけ見たい場合
        target_pars = ["mass_1","mass_2","luminosity_distance","geocent_time","phase","theta_jn","psi","a_1","a_2","tilt_1","tilt_2","phi_12","phi_jl","ra","dec"]

        # フィルタリング用の一時リスト
        filter_indices = []
        filtered_parnames = []
        
        # 現在のパラメータリスト(inf_pars)から、ターゲットを探す
        # (parnames と new_samples は inf_pars の順序に対応している前提)
        current_inf_pars = params['inf_pars'] 
        
        for i, p_name in enumerate(current_inf_pars):
            if p_name in target_pars:
                filter_indices.append(i)
                filtered_parnames.append(parnames[i]) # ラベルも対応するものだけ残す

        if len(filter_indices) > 0:
            # データをスライシングして上書き
            new_samples = new_samples[:, filter_indices]
            full_true_x = full_true_x[filter_indices]
            parnames = filtered_parnames
            print(f'... Plotting subset of parameters: {target_pars}')
        else:
            print('... Warning: target_pars not found in inf_pars. Plotting ALL parameters.')
        # ========================================================
...
# TF2 way of allowing GPU growth
gpus = tf.config.list_physical_devices('GPU')
if gpus:
    try:
        for gpu in gpus:
            tf.config.experimental.set_memory_growth(gpu, True)
        print('... letting GPU consumption grow as needed (TF2)')
...
class CVAE(tf.keras.Model):
    ...
    def __init__(self, x_dim, y_dim, n_channels, z_dim, n_modes, params):
        ...
        a = tf.keras.layers.Conv1D(filters=96, kernel_size=64, strides=1, kernel_regularizer=regularizers.l2(0.001), activation=self.act)(r1_input_y)
        a = tf.keras.layers.Conv1D(filters=96, kernel_size=32, strides=4, kernel_regularizer=regularizers.l2(0.001), activation=self.act)(a)
        a = tf.keras.layers.Conv1D(filters=96, kernel_size=32, strides=1, kernel_regularizer=regularizers.l2(0.001), activation=self.act)(a)
...
def run_vitc(params, x_data_train, y_data_train, x_data_val, y_data_val, x_data_test, y_data_test, y_data_test_noisefree, save_dir, truth_test, bounds, fixed_vals, bilby_samples, snrs_test=None):
    ...
    shutil.copy(os.path.join(os.path.dirname(__file__), 'new.py'),path)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 25 == 0:
        # ↓↓↓【ここから追加】↓↓↓
            print('... Cleaning up memory before reloading data')
            # 古いデータセットを消してメモリを解放
            if 'train_dataset' in locals():
                del train_dataset
            if 'x_data_train' in locals():
                del x_data_train
            if 'y_data_train' in locals():
                del y_data_train
            gc.collect()
            # ↑↑↑【ここまで追加】↑↑↑
