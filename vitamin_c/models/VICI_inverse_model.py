...
import corner
from vitamin_c.bilby_pe import *
import h5py

from vitamin_c.models.neural_networks import VICI_decoder
from vitamin_c.models.neural_networks import VICI_encoder
from vitamin_c.models.neural_networks import VICI_VAE_encoder
from vitamin_c.models.neural_networks import batch_manager
from vitamin_c.models import VICI_inverse_model
from vitamin_c.plots import *
import matplotlib
matplotlib.use('Agg')
...
def train(params, x_data, y_data, x_data_test, y_data_test, y_data_test_noisefree, y_normscale, save_dir, truth_test, bounds, fixed_vals, posterior_truth_test,snrs_test=None):    
    ...
    with graph.as_default():
        ...
        # =============================================================================================
        # COST FROM RECONSTRUCTION - Truncated Gaussian parts (Distance Weighting Added)
        # =============================================================================================
        # this sets up a loop over uncorrelated truncated Gaussians 
        ...
        if gauss_len > 0:
            ...
            # ---------------------------------------------------------
            # 【追加】距離パラメータの重み付け設定 (Python側での準備)
            # ---------------------------------------------------------
            # デフォルトの重みは全て 1.0
            weight_list_np = np.ones(gauss_len, dtype=np.float32)
            
            # 'luminosity_distance' が gauss_pars に含まれているか確認し、インデックスを取得
            target_dist_weight = 5.0  # ★距離に対する最大ペナルティ倍率（ここを調整！）
            
            if 'luminosity_distance' in params['gauss_pars']:
                dist_idx = params['gauss_pars'].index('luminosity_distance')
                print(f"Distance found at gauss index: {dist_idx}. Applying weight x{target_dist_weight}")
                # Pythonのリスト上で該当箇所だけ値を変更
                weight_list_np[dist_idx] = target_dist_weight
            else:
                print("Warning: 'luminosity_distance' not found in gauss_pars.")

            # TensorFlowの定数としてグラフに埋め込む
            base_weights = tf.constant(weight_list_np, dtype=tf.float32)

            # 学習が進むにつれて重みを適用する (Annealing)
            # ramp=0(初期)なら重み1.0, ramp=1(終盤)なら重み target_dist_weight になる
            # これにより初期学習の崩壊を防ぐ
            current_weights = 1.0 + (base_weights - 1.0) * ramp
            # ---------------------------------------------------------
            ...
            def truncnorm(i, lp):    # we set up a function that adds the log-likelihoods and also increments the counter
                ...
                # ★ここで重みを適用
                # i番目の重みを取り出す
                w = tf.slice(current_weights, [i], [1]) 
                
                # log_prob に重みを掛けて加算
                return [i+1, lp + tn.log_prob(pos) * w]

            # we do the loop until we've hit all the truncated gaussian parameters - i starts at 0 and the logprob starts at 0 
            _, reconstr_loss_gauss = tf.while_loop(
                lambda i, reconstr_loss_gauss: i < gauss_len, 
                truncnorm, 
                [0, tf.zeros([bs_ph], dtype=tf.dtypes.float32)]
            )
        ...
        mass_weight = 1.0 + 4.0 * ramp
        cost_R = -1.0*tf.reduce_mean(reconstr_loss_gauss + reconstr_loss_vonmise + mass_weight*reconstr_loss_masses + reconstr_loss_sky)
        #質量をより正確に推定するようにしてみる．
        # ========================================================
        # 【追加】チャープ質量による物理的制約 (Physics-Informed Loss)
        # ========================================================
        loss_chirp = 0.0
        chirp_weight = 5.0  # 重み係数 (効き目が薄ければ大きくする)

        if m1_len > 0 and m2_len > 0:
            # 1. 正規化パラメータ(bounds)を取得して定数化
            # (辞書のキー名はデータセット依存ですが、通常は 'mass_1_min' 等です)
            m1_min = tf.constant(bounds['mass_1_min'], dtype=tf.float32)
            m1_max = tf.constant(bounds['mass_1_max'], dtype=tf.float32)
            m2_min = tf.constant(bounds['mass_2_min'], dtype=tf.float32)
            m2_max = tf.constant(bounds['mass_2_max'], dtype=tf.float32)

            # 2. 予測値(Mean)を物理スケールに戻す [0,1] -> [min, max]
            # r2_xzy_mean_m1 などは、上で定義されている変数を使用
            m1_pred = r2_xzy_mean_m1 * (m1_max - m1_min) + m1_min
            m2_pred = r2_xzy_mean_m2 * (m2_max - m2_min) + m2_min

            # 3. 正解値(Truth)も物理スケールに戻す
            # x_ph から該当する列を抜き出す
            m1_true_norm = tf.boolean_mask(x_ph, m1_mask, axis=1)
            m2_true_norm = tf.boolean_mask(x_ph, m2_mask, axis=1)
            m1_true = m1_true_norm * (m1_max - m1_min) + m1_min
            m2_true = m2_true_norm * (m2_max - m2_min) + m2_min

            # 4. チャープ質量を計算する関数
            def get_chirp_mass(m1, m2):
                return (m1 * m2)**(0.6) / (m1 + m2)**(0.2)

            chirp_pred = get_chirp_mass(m1_pred, m2_pred)
            chirp_true = get_chirp_mass(m1_true, m2_true)

            # 5. 誤差を計算 (対数空間での二乗誤差がおすすめ)
            # 物理量の桁が大きいので、logを取って比率のズレを見るのが安定します
            loss_chirp = tf.reduce_mean(tf.square(tf.log(chirp_pred + 1e-12) - tf.log(chirp_true + 1e-12)))
        ...
        # THE VICI COST FUNCTION チャープ質量の重みを足してみます．
        COST = cost_R + ramp*KL + chirp_weight * loss_chirp #+ L1_weight_reg)
...
#        optimizer = tf.train.RMSPropOptimizer(params['initial_training_rate'])
    ...
    for i in range(params['num_iterations']):
        ...
        if i % params['report_interval'] == 0 and i > 0:
            ...
            plotdata.append([cost,kl,cost+kl,cost_val,kl_val,cost_val+kl_val,loss_chirp])
            ...
            try:
                ...
                plt.semilogx(xvec,np.array(plotdata)[:,6],label='loss_chirp',color='red',alpha = 0.5)
                plt.ylim([-25,15])
